...
function SyringeVisual({ fillPercent, units, totalUnits }: { fillPercent: number; units: string; totalUnits: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3" data-testid="visual-syringe">
      <div className="relative flex items-end gap-1.5">
        <div className="flex flex-col justify-between text-[10px] text-muted-foreground" style={{ height: barHeight }}>
          {ticks.map((i) => {
            ...
            return (
              <span key={i} className="leading-none text-right w-5">{val}</span>
            );
          })}
        </div>
        <div className="relative rounded-md border border-border" style={{ width: 32, height: barHeight }}>
          <div
            className="absolute bottom-0 left-0 right-0 bg-primary/15 transition-all duration-300 rounded-b-[3px]"
          ...
          {ticks.slice(1, -1).map((i) => {
            ...
            return (
              <div
                key={i}
                className="absolute left-0 w-1.5 border-t border-muted-foreground/20"
      ...
      <div className="text-center">
        <p className="text-base font-bold text-primary">{units} u</p>
        <p className="text-[10px] text-muted-foreground">{totalUnits}-unit</p>
      </div>
    </div>
  );
}
      ...
      <div className="container mx-auto px-4 py-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs">Peptide Amount (mg)</Label>
                      ...
                      <div className="flex gap-1.5">
                        {currentPreset.amounts.map((amt) => (
                          <Button
                            ...
                            size="sm"
                            className="flex-1"
                            onClick={() => setPeptideAmount(String(amt))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs">Water Volume (mL)</Label>
                    <Input
                      type="number"
                      min="0.1"
                      step="0.1"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <Select value={doseUnit} onValueChange={setDoseUnit}>
                        <SelectTrigger className="w-[80px]" data-testid="select-dose-unit">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs">Syringe Size</Label>
              ...
              <Card data-testid="card-results">
                ...
                <CardContent>
                  <div className="flex gap-6">
                    <div className="hidden sm:flex items-start justify-center pt-2">
                      <SyringeVisual
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-md bg-muted/50 p-3">
                          <p className="text-[11px] text-muted-foreground mb-0.5">Draw Volume</p>
                          <p className="text-lg font-bold" data-testid="text-draw-volume">{results.volumeToDrawMl} mL</p>
                        </div>
                        <div className="rounded-md bg-muted/50 p-3">
                          <p className="text-[11px] text-muted-foreground mb-0.5">Syringe Units</p>
                          <p className="text-lg font-bold" data-testid="text-syringe-units">{results.unitsToDraw}</p>
                          <p className="text-[10px] text-muted-foreground">on {results.totalUnits}-unit syringe</p>
                        </div>
                        <div className="rounded-md bg-muted/50 p-3">
                          <p className="text-[11px] text-muted-foreground mb-0.5">Doses / Vial</p>
                          <p className="text-lg font-bold" data-testid="text-total-doses">{results.totalDoses}</p>
                        </div>
                        {results.daysSupply && (
                          <div className="rounded-md bg-muted/50 p-3">
                            <p className="text-[11px] text-muted-foreground mb-0.5">Days Supply</p>
                            <p className="text-lg font-bold" data-testid="text-days-supply">{results.daysSupply}</p>
                          </div>
                        )}
                      </div>

                      <div className="rounded-md border border-border p-3">
                        <p className="text-[11px] text-muted-foreground mb-0.5">Concentration</p>
                      ...
                      {results.tooSmall && (
                        <div className="rounded-md bg-destructive/10 border border-destructive/20 p-3">
                          <p className="text-xs text-destructive flex items-center gap-1.5">
                            ...
                            Volume too small to measure accurately. Use less water.
                          </p>
                        </div>
                      )}
                      ...
                      {results.exceedsSyringe && (
                        <div className="rounded-md bg-destructive/10 border border-destructive/20 p-3">
                          <p className="text-xs text-destructive flex items-center gap-1.5">
                            <Info className="h-3.5 w-3.5 shrink-0" />
                            Exceeds syringe capacity. Use a larger syringe or less water.
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="rounded-md bg-muted/30 border border-border p-3 mt-4">
                    <p className="text-xs text-muted-foreground">
                      Reconstitute {peptideAmount} mg of {selectedPeptide} with {waterVolume} mL bacteriostatic water.
                      Draw to {results.unitsToDraw} units ({results.volumeToDrawMl} mL) for a {desiredDose} {doseUnit} dose.
                      {results.daysSupply && ` One vial lasts ~${results.daysSupply} days at your selected frequency.`}
                    </p>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-3 text-sm text-muted-foreground">
                <div>
                  <p className="font-medium text-foreground text-xs mb-0.5">1. Prepare</p>
                  <p className="text-xs">Clean vial tops with alcohol swabs. Ensure all materials are sterile.</p>
                </div>
                <div>
                  <p className="font-medium text-foreground text-xs mb-0.5">2. Add Solvent</p>
                  <p className="text-xs">Inject water slowly along the glass wall. Do not spray directly on powder.</p>
                </div>
                <div>
                  <p className="font-medium text-foreground text-xs mb-0.5">3. Mix Gently</p>
                  <p className="text-xs">Roll the vial gently between fingers. Never shake vigorously.</p>
                </div>
                <div>
                  <p className="font-medium text-foreground text-xs mb-0.5">4. Store</p>
                  <p className="text-xs">Refrigerate at 2-8 C. Use within 3-4 weeks.</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2.5 text-sm text-muted-foreground">
                <div>
                  <p className="font-medium text-foreground text-xs">Bacteriostatic Water</p>
                  <p className="text-[11px]">Contains 0.9% benzyl alcohol. Allows multi-use.</p>
                </div>
                <div>
                  <p className="font-medium text-foreground text-xs">Sterile Water</p>
                  <p className="text-[11px]">Preservative-free. Single-use only.</p>
                </div>
                <div>
                  <p className="font-medium text-foreground text-xs">Bacteriostatic NaCl</p>
                  <p className="text-[11px]">Isotonic saline. Reduces irritation.</p>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-[10px] text-muted-foreground max-w-xl mx-auto">
            For educational and research purposes only. All peptides are strictly for in-vitro research. Not for human consumption.
          </p>
        </div>
      </div>
    </div>
